...
def add_register_parser(parse):
    pass

def add_check_parser(parse):
    pass
...
class TagTool(CommandLineInterface):
    def __init__(self):
        ...
        add_register_parser(subparsers.add_parser(
                'register',help='register ndef tag'))
        add_check_parser(subparsers.add_parser(
                'check',help='check ndef tag'))
        ...
        self.rdwr_commands = {"show": self.show_tag,
                              ...
                              "register":self.register_tag,
                              "check":self.check_tag,}
    ...
    def register_tag(self,tag):
        newPmm = binascii.hexlify(tag.pmm)
        if newPmm == "033242828247aaff":
            print("Input name : ")
            name = raw_input()
            print ("Input passwd : ")
            passwd = raw_input()
            if passwd == "ss_laboratory":
                newTag = binascii.hexlify(tag.idm)
                connector = MySQLdb.connect(host = "localhost",user = "root",passwd = "ss_laboratory",db = "table1",charset = "utf8")
                sql = "SELECT * FROM table1"
                cursor = connector.cursor()
                num = int(cursor.execute(sql))
                sql = "SELECT * FROM table1 where idm = " + newTag
                if cursor.execute(sql) != 0:
                    print ("you have alredy registered")
                else:
                    sql = "INSERT INTO table1 VALUES(" + str(num+1) + ",'" + name + "'," + newTag + ")"
                    cursor.execute(sql)
                    connector.commit()

                    #sql = "DELETE FROM table1 WHERE id not IN(SELECT max_id FROM(SELECT MAX(id) max_id FROM table1 GROUP BY idm)tmp)"
                    #cursor.execute(sql)
                    #connector.commit()
                    cursor.close()
                    connector.close()

                    print ("save " + name + "\'s data")
            else :
    ...
    def show_tag(self, tag):
        ...
        #for x in inspect.getmembers(tag, inspect.ismethod):
        #    print (x[0])
